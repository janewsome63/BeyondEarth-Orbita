...
@OnlyIn(Dist.CLIENT)
public class AATVGuiWindow extends AbstractContainerScreen<AATVGui.GuiContainer> {

    private static final ResourceLocation texture = new ResourceLocation(BeyondOrbitaMod.MODID, "textures/screens/aatv.png");

    public AATVGuiWindow(AATVGui.GuiContainer container, Inventory inventory, Component text) {
    ...
    public void render(PoseStack ms, int mouseX, int mouseY, float partialTicks) {
        ...
        int fuel = menu.aatv.getEntityData().get(AATVEntity.FUEL);

        if (!CompatibleManager.JEI.isLoaded() && GuiHelper.isHover(this.getFluidBounds(), mouseX - this.leftPos, mouseY - this.topPos)) {
            fuelToolTip.add(GaugeTextHelper.buildBlockTooltip(GaugeTextHelper.getStorageText(GaugeValueHelper.getFuel(fuel, AATVEntity.FUEL_BUCKETS * FluidUtil2.BUCKET_SIZE)), ChatFormatting.WHITE));
    ...
    protected void renderBg(PoseStack ms, float p_97788_, int p_97789_, int p_97790_) {
        ...
        FluidStack fluidStack = new FluidStack(FluidsRegistry.KEROSENE_BLOCK.get().getFluid(), fuelGaugeValue.getAmount());
    ...
    public IGaugeValue getFuelGaugeValue() {
        int fuel = menu.aatv.getEntityData().get(AATVEntity.FUEL);
        return GaugeValueHelper.getFuel(fuel, AATVEntity.FUEL_BUCKETS * FluidUtil2.BUCKET_SIZE);
    }

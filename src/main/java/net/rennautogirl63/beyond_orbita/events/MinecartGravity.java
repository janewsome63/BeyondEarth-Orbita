...
public class MinecartGravity {
    ...
    public static void gravitySystem(Entity entity, float gravity, float drag) {
        ...
        float drag_coeff = 0.95F;

        gravity = gravity * grav_coeff;
        drag = 1.0F - drag;
        Block[] rails = {Blocks.RAIL, Blocks.POWERED_RAIL, Blocks.DETECTOR_RAIL, Blocks.ACTIVATOR_RAIL, ForgeRegistries.BLOCKS.getValue(new ResourceLocation("create:controller_rail"))};
        Block[] air = {Blocks.AIR, Blocks.VOID_AIR, Blocks.CAVE_AIR};

        if (gravity == 0.0F) {
            if (!entity.isNoGravity()) {
                entity.setNoGravity(true);
            }
            if (Arrays.asList(air).contains((level.getBlockState(new BlockPos(x, y - 0.99, z))).getBlock())) {
            ...
            if (!entity.isOnGround() && !Arrays.asList(rails).contains((level.getBlockState(new BlockPos(x, y, z))).getBlock())) {
